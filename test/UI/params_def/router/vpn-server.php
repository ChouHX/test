...
if(!nvram.vpn_server_eas){
	nvram.vpn_server_eas = '';
}

if(!nvram.vpn_server_dns){
	nvram.vpn_server_dns = '';
}
...
if(!nvram.lan3_ifname){
	nvram.lan3_ifname = [];
		...
		tabs = [['server1', $lang.SERVER + ' 1 <i class="icon-tools"></i>'],['server2', $lang.SERVER + ' 2 <i class="icon-tools"></i>']];

for (i = 0; i < tabs.length; ++i) {
	t = tabs[i][0];
	if(!nvram['vpn_'+ t +'_ccd_val']){
		nvram['vpn_'+ t +'_ccd_val'] = [];
	}
	if(!nvram['vpn_'+ t +'_users_val']){
		nvram['vpn_'+ t +'_users_val'] = [];
	}
}

		sections = [['basic', '<i class="icon-system"></i> ' + $lang.BASIC],['advanced', '<i class="icon-shield"></i> ' + $lang.ADVANCED],['keys','<i class="icon-lock"></i> ' + $lang.KEYS]];
		ccdTables = [];
		usersTables = [];
		statusUpdaters = [];
		for (i = 0; i < tabs.length; ++i)
	...
	<form id="_fom" method="post" action="tomato.cgi">
		...
		<div class='heading'><script type="text/javascript">document.write($lang.OPEN_VPN_SERVER)</script></div><hr>
		<div class="section" id="htmlConf">
			<script type="text/javascript">
				...
				for (i = 0; i < tabs.length; ++i)
				{
					...
					var action = (eval('vpn'+(i+1)+'up') ? 'title="Stop VPN Server ' + (i+1) + '">' : 'title="Start VPN Server ' + (i+1) + '">');
					var status = (!eval('vpn'+(i+1)+'up') ? '' : '');

					htmlOut += '</ul>'
					+ '<div class="box"><div class="heading">'+ $lang.VPN_SERVER +' #'+(i+1) + status + ' <a id="_vpn' + t + '_button" class="pull-right" href="#" data-toggle="tooltip"' +
					action + '</a></div><div class="content">';

					htmlOut += '<div id=\''+t+'-basic\'>';
					htmlOut += createFormFields([
						{ title: $lang.VIA_INTERNET, name: 'f_vpn_'+t+'_eas', type: 'checkbox', value: nvram.vpn_server_eas.indexOf(''+(i+1)) >= 0 },
						{ title: $lang.INTERFACE_TYPE, name: 'vpn_'+t+'_if', type: 'select', options: [ ['tap','TAP'], ['tun','TUN'] ], value: eval( 'nvram.vpn_'+t+'_if' ) },
						{ title: $lang.BRIDGE_TAP_WITH, indent: 2, name: 'vpn_'+t+'_br', type: 'select', options: [
							['br0','LAN (br0)*'],
							['br1','LAN1 (br1)'],
							['br2','LAN2 (br2)'],
							['br3','LAN3 (br3)']
							], value: eval ( 'nvram.vpn_'+t+'_br' ), suffix: ' <small>* default</small> ' },
						{ title: $lang.PROTOCOL, name: 'vpn_'+t+'_proto', type: 'select', options: [ ['udp4','UDP'], ['tcp4-server','TCP'] ], value: eval( 'nvram.vpn_'+t+'_proto' ) },
						{ title: $lang.SERVER_PORT, name: 'vpn_'+t+'_port', type: 'text', value: eval( 'nvram.vpn_'+t+'_port' ) },
						{ title: $lang.FIREWALL, name: 'vpn_'+t+'_firewall', type: 'select', options: [ ['auto', 'Automatic'], ['external', 'External Only'], ['custom', 'Custom'] ], value: eval( 'nvram.vpn_'+t+'_firewall' ) },
						...
						{ title: $lang.EXTRA_HMAC_AUTHORIZATION, name: 'vpn_'+t+'_hmac', type: 'select', options: [ [-1, 'Disabled'], [2, 'Bi-directional'], [0, 'Incoming (0)'], [1, 'Outgoing (1)'] ], value: eval( 'nvram.vpn_'+t+'_hmac' ) },
						{ title: $lang.VPN_SUBNET_NETMASK, multi: [
						...
						{ title: $lang.CLIENT_ADDRESS_POOL, multi: [
					...
					htmlOut += createFormFields([
						...
						{ title: $lang.PUSH_LAN_TO_CLIENTS, name: 'f_vpn_'+t+'_plan', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_plan' ) != 0 },
						{ title: $lang.DIRECT_CLIENTS_TO_REDIRECT_INTERNET_TRAFFIC, name: 'f_vpn_'+t+'_rgw', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_rgw' ) != 0 },
						{ title: $lang.RESPOND_TO_DNS, name: 'f_vpn_'+t+'_dns', type: 'checkbox', value: nvram.vpn_server_dns.indexOf(''+(i+1)) >= 0 },
						{ title: $lang.ADVERTISE_DNS_TO_CLIENTS, name: 'f_vpn_'+t+'_pdns', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_pdns' ) != 0 },
						...
						{ title: $lang.MANAGE_CLIENT_SPECIFIC_OPTIONS, name: 'f_vpn_'+t+'_ccd', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_ccd' ) != 0 },
						{ title: $lang.ALLOW_CLIENT + '<->' + $lang.CLIENT, name: 'f_vpn_'+t+'_c2c', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_c2c' ) != 0 },
						{ title: $lang.ALLOW_CLIENT, name: 'f_vpn_'+t+'_ccd_excl', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_ccd_excl' ) != 0 },
						{ title: '', suffix: '<table class=\'line-table\' id=\'table_'+t+'_ccd\'></table>' },
						{ title: $lang.ALLOW_USER_PASS_AUTH, name: 'f_vpn_'+t+'_userpass', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_userpass' ) != 0 },
						{ title: $lang.ALLOW_ONLY_USER_PASS_AUTH, name: 'f_vpn_'+t+'_nocert', type: 'checkbox', value: eval( 'nvram.vpn_'+t+'_nocert' ) != 0 },
						{ title: '', suffix: '<table class=\'line-table\' id=\'table_'+t+'_users\'></table>' },
						{ title: $lang.CUSTOM_OPTIONS, name: 'vpn_'+t+'_custom', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_custom' ), style: 'width: 100%; height: 80px;' }
					]);
					htmlOut += '</div>';
					...
					htmlOut += createFormFields([
						...
						{ title: $lang.SERVER_CERTIFICATE, name: 'vpn_'+t+'_crt', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_crt' ), style: 'width: 100%; height: 80px;' },
						{ title: $lang.SERVER_KEY, name: 'vpn_'+t+'_key', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_key' ), style: 'width: 100%; height: 80px;' },
						{ title: $lang.DIFFIE_HELLMAN_PARAMETERS, name: 'vpn_'+t+'_dh', type: 'textarea', value: eval( 'nvram.vpn_'+t+'_dh' ), style: 'width: 100%; height: 80px;' }
					]);
					htmlOut += '</div>';
		...
		<!-- <button type="button" value="Cancel" id="cancel-button" onclick="javascript:reloadPage();" class="btn btn-primary">Cancel <i class="icon-cancel"></i></button> -->
