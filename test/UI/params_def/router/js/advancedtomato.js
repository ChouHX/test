...
function AdvancedTomato() {
	 ...
	 ************************************************************************************************/
	// Call navigation function in dt.js to generate navigation
	navi();
	...
	$( '.navigation > ul > li > a' ).on( gui.nav_action, function() {
		...
		function reveal_navigation() {
			...
			elem = elm.parent('li').next();
			while(elem.children('a').text())
			{
				//if (elem.hasClass( 'active' ) ) { continue; } 
				if(webmx.indexOf(navi_menu(trimLeft(elem.children('a').text()))) == -1&&elem.hasClass( 'active' ))
				{
						elem.removeClass( 'active' ).find( 'ul' ).slideUp( '100' );
				}
				elem=elem.next();
			}
			elem = elm.parent('li').prev();	
				//alert(navi_menu(elem.children('a').text()));
			while(elem.children('a').text())
			{
				//if (elem.hasClass( 'active' ) ) { continue; } 
				if(webmx.indexOf(navi_menu(trimLeft(elem.children('a').text()))) == -1&&elem.hasClass( 'active' ))
	...
	$( '.toggle-nav' ).on( 'click', function() {
		if ( !$( '.navigation' ).hasClass( 'collapsed' ) ) {
			...
			$( '.top-header .logo').css('background-image','url()');
			...
			$( '.top-header .logo').css('background-image','url(detran.png)');
	...
	$( '[data-box]' ).each( function() {
		...
		$( html ).on( 'click', function() {
			...
			if ( status ) {
				...
				$( parent ).find( '.content' ).hide(); //stop( true, true ).slideUp( 700, 'easeOutBounce' );
				...
				$( parent ).find( '.content' ).show(); //stop( true, true ).slideDown( 350, 'easeInQuad' );
...
function loadPage( page, is_history ) {
	...
	$.ajax( { url: page, async: true, cache: false, timeout: 5950 } )
		.done( function( resp ) {
			...
			if ( ajax_retries <= 8 ) {
				...
				if ( $( 'body .body-overwrite' ).length == 0 ) {
					...
					$( 'body' ).append( '<div class="body-overwrite"><div class="body-overwrite-text text-center"><div class="spinner spinner-large"></div>' +
					                    '<br><br><b><% translate("Connection lost"); %>!</b><br><% translate("Attempting to reconnect"); %>...</div></div>' );
