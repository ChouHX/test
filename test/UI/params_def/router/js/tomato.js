...
function v_ascii(e, quiet, msg)
{
	if(arguments.length == 2)
	{
		if((E('lb'+e) == null) || (E('lb'+e).innerText == ''))
		{
			msg = $lang.INVALID_ASCII_CHARACTERS;
		}
		else
		{
			msg = $lang.INVALID + '<' + E('lb'+e).innerText + '>';
		}
	}
...
function _v_range(e, quiet, min, max, name)
{
	...
	if ((!v.match(/^ *[-\+]?\d+ *$/)) || (v < min) || (v > max)) {
		ferror.set(e, $lang.THE_EFFECTIVE_RANGE_IS + ': ' + min + '-' + max, quiet);
		return 0;
	}
	e.value = v * 1;
	ferror.clear(e);
...
function v_mins(e, quiet, min, max)
{
	...
	ferror.set(e, $lang.INVALID_MINUTES + '.', quiet);
	return 0;
}
...
function v_macip(e, quiet, bok, lan_ipaddr, lan_netmask)
{
	...
	if ((a = fixMAC(s)) != null) {
		if (isMAC0(a)) {
			if (bok) {
				...
				ferror.set(e, $lang.INVALID + 'MAC' + $lang.OR + $lang.VAR_IP);
				return false;
			}
		}
		else e.value = a;
	...
	if (a.length > 2) {
		ferror.set(e, $lang.INVALID_IP_ADDRESS_RANGE, quiet);
		return false;
	}
	...
	for (i = 0; i < a.length; ++i) {
		...
		b = fixIP(b);
		if (!b) {
			ferror.set(e, $lang.INVALID_IP_ADDRESS, quiet);
			return false;
		}

		if ((aton(b) & aton(lan_netmask))!=(aton(lan_ipaddr) & aton(lan_netmask))) {
			ferror.set(e, $lang.VAR_IP + $lang.EXCEED + 'LAN', quiet);
			return false;
		}
...
function v_dns(e, quiet)
{
	...
	if (e.value == '') {
		...
		if (s.length == 1) {
			...
			ferror.set(e, $lang.INVALID_IP_ADDRESS_OR_PORT, quiet);
			return false;
		}
		...
		if ((s[1] = fixPort(s[1], -1)) == -1) {
			ferror.set(e, $lang.INVALID_PORT, quiet);
			return false;
		}
...
function _v_iptip(e, ip, quiet,s)
{
	var ma, x, y, z, oip;
	var a, b;
	...
	if (ip.match(/^(.*)-(.*)$/)) {
		...
		b = fixIP(RegExp.$2);
		if ((a == null) || (b == null)) {
			ferror.set(e, $lang.INVALID_IP_ADDRESS_RANGE + '-' + oip, quiet);
			return null;
		}
		ferror.clear(e);
	...
	if (ip.match(/^(.*)\/(.*)$/)) {
		...
		if (isNaN(ma)) {
			...
			if ((ma == null) || (!_v_netmask(ma))) {
				ferror.set(e, $lang.INVALID_SUBNET_MASK + '-' + oip, quiet);
				return null;
			}
		}
		else {
	...
	ip = fixIP(ip);
	if (!ip) {
		ferror.set(e, $lang.INVALID_IP_ADDRESS + '-' + oip, quiet);
		return null;
	}
...
function v_iptip(e, quiet, multi)
{
	...
	if (multi) {
		if (v.length > multi) {
			ferror.set(e, $lang.TOO_MANY_IP_ADDRESSES_ENTERED, quiet);
			return 0;
		}
	}
	else {
...
function _v_domain(e, dom, quiet)
{
	...
	if (s.length > 0) {
		...
		if (s == null) {
			ferror.set(e, $lang.VDOMAIN_ALERT, quiet);
			return null;
		}
	}
	ferror.clear(e);
...
function _v_ipv6_addr(e, ip, ipt, quiet)
{
	...
	if ((ipt) && ip.match(/^([A-Fa-f0-9:]+)\/(\d+)$/)) {
		...
		if ((a == null) || (b == null)) {
			ferror.set(e, oip + '-' + $lang.INVALID_IPV6_ADDRESS, quiet);
			return null;
		}
		if (b < 0 || b > 128) {
...
function _v_portrange(e, quiet, v)
{
	if (v.match(/^(.*)[-:](.*)$/)) {
		...
		y = fixPort(y, -1);
		if ((x == -1) || (y == -1)) {
			ferror.set(e, $lang.INVALID_PORT_RANGE + ':' + v, quiet);
			return null;
		}
		if (x > y) {
			v = x;
...
function v_iptport(e, quiet)
{
	...
	if (a.length == 0) {
		ferror.set(e, $lang.NEED_TO_SPECIFY_PORT_LIST_OR_PORT_RANGE + '.', quiet);
		return 0;
	}
	if (a.length > 10) {
		ferror.set(e, $lang.ONLY_10_PORTS_RANGE_SETS_ARE_ALLOWED + '.', quiet);
		return 0;
	}
...
function v_netmask(e, quiet)
{
	...
	ferror.set(e, $lang.INVALID_SUBNET_MASK, quiet);
	return 0;
}
...
function v_length(e, quiet, min, max)
{
	...
	if (n < min) {
		ferror.set(e, $lang.INVALID_LENGTH_PLEASE_ENTER_AT_LEAST + min + $lang.CHARACTERS + (min == 1 ? '.' : 's.'), quiet);
		return 0;
	}
	max = max || e.maxlength;
	if (n > max) {
		ferror.set(e, $lang.INVALID_LENGTH_PLEASE_ENTER_LESS_THAN + max + $lang.CHARACTERS, quiet);
		return 0;
	}
	e.value = s;
	ferror.clear(e);
...
function _v_iptaddr(e, quiet, multi, ipv4, ipv6,s)
{
	var v, t, i;
    if(typeof(s)=='undefined'){
        s = 1;
	...
	if (multi) {
		...
		if (v.length > 1) {
			ferror.set(e, $lang.INVALID_DOMAIN_NAME_OR_IP_ADDRESS, quiet);
			return 0;
		}
	}
...
function _v_hostname(e, h, quiet, required, multi, delim, cidr)
{
	...
	if (multi) {
		...
		if (v.length > 1) {
			ferror.set(e, $lang.INVALID_HOSTNAME + '.', quiet);
			return null;
		}
	}
	...
	for (i = 0; i < v.length; ++i) {
		...
		if (s.length > 0) {
			...
			if (s.search(re) == -1 || s.search(/^\d+$/) != -1) {
				ferror.set(e, $lang.INVALID_HOSTNAME + '.' + $lang.A_Z_AND_0_9_IN_THE_MIDDLE_ARE_ALLOWED + '.', quiet);
				return null;
			}
		} else if (required) {
			ferror.set(e, $lang.INVALID_HOSTNAME + '.', quiet);
...
function v_nodelim(e, quiet, name, checklist)
{
	...
	if (e.value.indexOf('<') != -1 ||
		...
		ferror.set(e, $lang.NODELIM_ALERT, quiet);
		return 0;
	}
	ferror.clear(e);
	return 1;
...
TomatoGrid.prototype = {
	...
	insert: function(at, data, cells, escCells) {
		...
		if ((this.canMove) || (this.canEdit) || (this.canDelete)) {
			...
			if (this.canEdit) e.title = $lang.CLICK_TO_EDIT; $(e).css('cursor', 'text');
		}
	...
	createControls: function(which, rowIndex) {
		...
		if (which == 'edit') {
			c.innerHTML = '<button type="button" class="btn btn-danger" value="Delete" onclick="TGO(this).onDelete()">' + $lang.VAR_DEL + '<i class="icon-cancel"></i></button> ' +
			'<button type="button" class="btn" value="Cancel" onclick="TGO(this).onCancel()">' + $lang.UNSET + '<i class="icon-disable"></i></button> ' +
			'<button type="button" class="btn btn-primary" value="OK" onclick="TGO(this).onOK()">' + $lang.VAR_BTN_SURE + '<i class="icon-check"></i></button>';
		}
		else {
			c.innerHTML =
			'<button type="button" class="btn btn-danger" value="Add" onclick="TGO(this).onAdd()">' + $lang.VAR_ADD + '<i class="icon-plus"></i></button>';
		}
...
TomatoRefresh.prototype = {
	...
	toggle: function(delay) {
		var tomato_status_overview_refresh;
		if (this.running) this.stop();
		else this.start(delay);
		if (getCookie("tomato_status_overview_refresh") == 0){
		  $('.spinner-small').hide();
		}else{
		  $('.spinner-small').show();
		}
	...
	updateUI: function(mode) {
		...
		if ((e = E('refresh-button')) != null) {
			e.innerHTML = b ? $lang.VAR_TRACK_STOP + '<i class="icon-cancel"></i>' : $lang.VAR_REFRESH + '<i class="icon-refresh"></i>';
...
function genStdTimeList(id, zero, min)
{
	...
	if (min >= 0) {	
		...
		for (i = 0; i < t.length; ++i) {
			...
			if (v == 60) b.push(1 + $lang.MINUTES);
			else if (v > 60) b.push((v / 60) + $lang.MINUTES);
				else if (v == 1) b.push(1 + $lang.VAR_SECOND);
					else b.push(v + $lang.VAR_SECOND);
		}
		b.push('</select> ');
	}
	return b.join('');
...
function genStdRefresh(spin, min, exec)
{
	...
	html += genStdTimeList('refresh-time', $lang.MANUAL +  $lang.VAR_REFRESH, min);
	html += '<button value="Refresh" onclick="' + (exec ? exec : 'refreshClick()') + '; return false;" id="refresh-button" class="btn">'+ $lang.VAR_REFRESH +' <i class="icon-refresh"></i></button></div>';
	return html;
}
...
function navi_icons ($name) {
		switch ($name) {
		case $lang.DEVICE_STATUS: 			return 'icon_yulan'; break;
		case $lang.BASIC_NETWORK: 	        return 'guanfangbanben'; break;
		case $lang.WLAN_SETTING: 		    return 'wifi'; break;
		case $lang.ADVANCED_NETWORK: 	    return 'wumoxing'; break;
		case $lang.FIREWALL: 			    return 'yunyingguanli'; break;
		case 'Debugging': 					return 'tiaoshi'; break;
		case 'Web Services': 				return 'fuwudiqiu'; break;
		case $lang.VPN_MODE: 		        return 'peiwangyindao'; break;
		case $lang.VAR_MENU_SYSCFG: 	    return 'jichuguanli'; break;
		default: 							return 'plus'; break;
	}	
}
function navi_menu ($name) {
		switch ($name) {
		case $lang.DEVICE_STATUS: 			return 'status'; break;
		case $lang.BASIC_NETWORK: 		    return 'basic'; break;
		case $lang.WLAN_SETTING: 			return 'wlan'; break;
		case $lang.ADVANCED_NETWORK:  	    return 'forward'; break;
		case $lang.FIREWALL: 				return 'firewall'; break;
		case $lang.VPN_MODE: 			    return 'vpn'; break;
		case $lang.VAR_MENU_SYSCFG: 	    return 'admin'; break;
...
function navi() {
	...
	var menu      = {
		'<%translate("Status");%>': 		{
		'<%translate("Overview");%>':		'status-home.php',
//		'<%translate("VPN");%>':			'status-vpn.php',
//		'<%translate("LAN");%>':			'status-wlan.php',
		'<%translate("Traffic Stats.");%>':			'status-traffic.php',
		'<%translate("GPS Status");%>':			'status-gps.php',
		'<%translate("Device List");%>':	'status-devices.php'
		},
		'<%translate("Basic Network");%>': 	{
/* WAN-BEGIN */
			'<%translate("WAN");%>':		'basic-wan.php',
/* WAN-END */
			'<%translate("Cellular");%>': 	'basic-cellular.php',
			'<%translate("Cellular");%> 2': 'basic-cellular2.php',
			'<%translate("LAN");%>':		'basic-lan.php',
/* IPV6-BEGIN */
			'<%translate("IPv6");%>':               'basic-ipv6.php',
/* IPV6-END */
			'<%translate("VLAN");%>':                       'basic-vlan.php',
			'<%translate("Schedule");%>':                  'basic-schedule.php',
			'<%translate("DDNS");%>':		'basic-ddns.php',
			'<%translate("Routing");%>':	'basic-routing.php'
		},
/* WIFI-BEGIN */		
		'<%translate("WLAN");%>':{
			'<%translate("Basic Settings");%>':		'wlan-network.php',
			'<%translate("MultiSSID");%>':		'wlan-network_vif.php',
			'<%translate("Wireless Survey");%>':	'wlan-survey.php'	
			},
/* WIFI-END */		
		'<%translate("Advanced Network");%>': {
			'<%translate("Port Forwarding");%>':	'forward-basic.php',
/* IPV6-BEGIN */
			'<%translate("IPv6 Port Forwarding");%>':           'forward-basic-ipv6.php',
/* IPV6-END */
			'<%translate("Port Redirecting");%>':	'forward-redirection.php',
			'<%translate("DMZ");%>':				'forward-dmz.php',
/* IPPASS-BEGIN */
			'<%translate("IP Passthrough");%>':		'forward-ippass.php',
/* IPPASS-END */
			'<%translate("Triggered");%>'	:		'forward-triggered.php',
			'<%translate("Captive Portal");%>':		'forward-portal.php',
			//'Captive Portal2':		'forward-splashd.php',
			'<%translate("Serial App.");%>': 		'forward-dtu.php',
			'<%translate("GPS");%>':				'forward-gps.php',
			'<%translate("Serial App2.");%>': 		'forward-dtu2.php',
			'<%translate("AT over IP");%>':			'forward-atoip.php',
/* TOR-BEGIN */
			'<%translate("TOR Project");%>':							'forward-tor.php',
/* TOR-END */
			'<%translate("UPnP/NAT-PMP");%>':		'forward-upnp.php',
			'<%translate("Bandwidth Control");%>':	'forward-bwlimit.php',
			'<%translate("VRRP");%>':				'forward-vrrp.php',
			'<%translate("Static DHCP");%>':		'forward-static.php'

		},
		
		'<%translate("Firewall");%>': {
			'<%translate("IP/URL Filtering");%>': 	'firewall-port_filter.php',
			'<%translate("Domain Filtering");%>':   'firewall-content_filter.php'
		},
/* VPN-BEGIN */
		'<%translate("VPN Tunnel");%>': {
			'<%translate("GRE");%>':				'vpn-gre.php',
/* OPENVPN-BEGIN */
			'<%translate("OpenVPN Client");%>':			'vpn-client.php',
/* OPENVPN-END */
/* OPENVPNSERVER-BEGIN */
			'<%translate("OpenVPN Server");%>':			'vpn-server.php',
/* OPENVPNSERVER-END */
			'<%translate("PPTP/L2TP Server");%>':			'vpn-pptp-server.php',
/* NOCAT-BEGIN */
			'<%translate("PPTP Online");%>':			'vpn-pptp-online.php',
/* NOCAT-END */
/* PPTPD-BEGIN */	
			'<%translate("PPTP/L2TP Client");%>':			'vpn-xtp.php',
/* PPTPD-END */
			'<%translate("L2TP V3");%>':			'vpn-l2tpv3.php',
/* SSHVPN-BEGIN */
			'<%translate("SSH-VPN Client");%>':			'vpn-ssh.php',
/* SSHVPN-END */
/* IPSEC-BEGIN */
			'<%translate("IPSec");%>':			'vpn-ipsec.php',
			'<%translate("DMVPN");%>':                      'vpn-dmvpn.php'
/* IPSEC-END */
		},
/* VPN-END */
		'<%translate("Administration");%>': {
			
			'<%translate("Identification");%>':		'admin-ident.php',
			'<%translate("Time");%>':				'admin-time.php',
			'<%translate("Admin Access");%>':		'admin-access.php',
			'HTTPS <%translate("Certificate");%>':		'admin-cert.php',
			'<%translate("Scheduled Reboot");%>':   'admin-sched.php',
			
/* SNMP-BEGIN */
			'<%translate("SNMP");%>':		    'admin-snmp.php',
/* SNMP-END */
			'<%translate("Storage Settings");%>':		'admin-filelist.php',
			'<%translate("M2M Settings");%>':		'admin-m2m.php',
      '<%translate("TR-069");%>':         'admin-tr069.php',
			'<%translate("DI/DO Setting");%>':	'admin-gpctl.php',
			'<%translate("Configuration");%>':		'admin-config.php',
/* NOCAT-BEGIN */
			'<%translate("Scripts");%>':			'admin-scripts.php',
/* NOCAT-END */
			'<%translate("Logging");%>':			'admin-log.php',
			'<%translate("Upgrade1");%>':			'admin-upgrade.php',
			'<%translate("MCU Upgrade");%>':		'admin-mcuupgrade.php'
	...
	if ( activeURL == null || activeURL == '' ) activeURL = '#status-home.php';

	// Loop Through MENU
	$.each( menu, function( key, linksobj ) {

		var category  = '';
		var groupname = '<i class="iconfont iconf-' + navi_icons( key ) + '"></i> <span class="icons-desc">' + key + '</span>';
		if(bi.wlof == '1')
		{
			if(navi_icons( key ) == 'wifi')
			{
				return true;
			}
		}
		...
		$.each( linksobj, function( name, link ) {
			...
			if(webmx.indexOf(navi_menu(key)) != -1)
			{ 
				category += '<li class="' + ((activeURL == link) ? 'active' : 'active') + '"><a href="' + link + '">' + name + '</a></li>'; 				
			}	
			else 
			{ 
				if(bi.hw != 'dd' && link == '#basic-cellular2.php')
				{
					return true;
				}
				if(bi.mcu == '0' && link == '#admin-mcuupgrade.php')
				{
					return true;
				}
				//R10 has no wan function
				if(bi.waof == '1')
				{
					if(link == '#basic-wan.php')
					{
						return true;
					}
				}
				if(bi.r21 != '1')
				{
					if(link == '#admin-gpctl.php')
					{
						return true;
					}
				}
				//G92 new board or Rx serial board no mcu
				if(bi.r_type.indexOf("V2") != -1 || bi.r_type.indexOf("R") != -1)
				{
					if(link == '#admin-mcuupgrade.php')
					{
						return true;
					}
				}
				//add by Micheal for init
				//MT7621
				if(bi.r_type.indexOf("G") != -1)
				{
					if(bi.r_type.indexOf("V2") != -1)
					{
						if(bi.gps == 'n')
						{
							if(link == '#forward-gps.php' || link == '#status-gps.php')
							{
								return true;
							}
						}
						//G20 only has two serial, Uart0 and Uart 2, Uart1 is not use
						if(bi.r_type.indexOf("G2") != -1)
						{
							if((link == '#forward-dtu2.php') || (link == '#admin-gpctl.php'))
							{
								return true;
							}
						}
					...
					else //7621 V3
					{
						...
						if((bi.r_type.indexOf("G50") != -1) || (bi.r_type.indexOf("G20") != -1))
						{
							if(link == '#admin-gpctl.php')
							{
								return true;
							...
							{
								if((link == '#forward-dtu2.php') || (link == '#forward-dtu.php'))
								{
									return true;
								}
							}
						...
						else if(bi.r_type.indexOf("G51") != -1)
						{
							if(bi.gps == 'n')
							{
								if(link == '#forward-dtu.php')
							...
							else if(bi.gps == 'e')//device has two serials, only ttyUSB100 can use to dtu function
							{
								if(link == '#forward-dtu2.php')
								{
									return true;
				...
				else//MT7628
				{
					if(bi.r21 == '1')
					{
						...
						else if(bi.gps == 'i')//device has two serials, ttyS0 and ttyUSB100 both can use to dtu function
						{
							if(link == '#forward-dtu.php')
							{
								return true;
					...
					{
						//add new type CM20, has no serial
						if(bi.r_type.indexOf("CM20") != -1)
						{
							if(link == '#forward-dtu2.php' || link == '#forward-dtu.php')
							{
								return true;
						...
						else
						{
							if(bi.gps == 'n')
							{
								...
								//R10D has two serial, ttyS0 and ttyUSB100
							...
							else if(bi.gps == 'e')//device has one serial, none can use to dtu function
							{
								if(link == '#forward-dtu2.php' || link == '#forward-dtu.php')
								{
									return true;
							...
							else if(bi.gps == 'i')//device has one serials, ttyS0 can use to dtu function
							{
								//R10D has two serial, ttyS0 and ttyUSB100
								if(bi.r_type.indexOf("R10D") != -1)
								{
...
// 提交表单
function submit_form(form_id,page) {
	var names = [], vals = [];
	$('#'+form_id).find('input,select,textarea').each(function(i, o){
		if (o.name.substr(0, 1) == '_' || o.name.substr(0, 2) == 'f_' || !o.name) {
			return true;
		}
		if(page == 'forward-basic' && o.name != 'portforward'){
			return true;
		}
		names.push(o.name);
		vals.push(o.value);
...
function reloadPage() {
	if (document.location.hash.match(/#/)) { loadPage(document.location.hash); } else { loadPage('#status-home.php'); }
}

function reboot() {
	if (confirm($lang.RESTART_THE_ROUTER + "?")) { form.submitHidden('tomato.cgi', { _reboot_now: 1, _commit: 0, _nvset: 0 }); } else { return false; }
}

function hwreboot() {
	if (confirm($lang.RESTART_THE_ROUTER + "?")) { form.submitHidden('tomato.cgi', { _reboot: 0, _commit: 0, _nvset: 0,_hwreboot:1 }); } else { return false; }
}

function shutdown() {
	if (confirm($lang.TURN_OFF_ROUTER + "?")) { form.submitHidden('shutdown.cgi', { }); } else { return false; }
}

function logout(){
	form.submitHidden('logout.php', { });
}
