...
<content>
	<script type="text/javascript">
		...
		function restoreButton()
		{
			...
			if (!confirm('<% translate("Restore data from"); %> ' + name + '<% translate("?"); %>')) return false;
	...
	<div class="box">
		<div class="heading"><% translate("Bandwidth Monitoring Settings"); %></div>
		<div class="content">
			...
			<script type="text/javascript">
				...
				$('#_fom').forms([
					...
					{ title: '<% translate("Save Frequency"); %>', indent: 2, name: 'rstats_stime', type: 'select', value: nvram.rstats_stime, options: [
						[1,'<% translate("Every Hour"); %>'],[2,'<% translate("Every 2 Hours"); %>'],[3,'<% translate("Every 3 Hours"); %>'],[4,'<% translate("Every 4 Hours"); %>'],[5,'<% translate("Every 5 Hours"); %>'],[6,'<% translate("Every 6 Hours"); %>'],[9,'<% translate("Every 9 Hours"); %>'],[12,'<% translate("Every 12 Hours"); %>'],[24,'<% translate("Every 24 Hours"); %>'],[48,'<% translate("Every 2 Days"); %>'],[72,'<% translate("Every 3 Days"); %>'],[96,'<% translate("Every 4 Days"); %>'],[120,'<% translate("Every 5 Days"); %>'],[144,'<% translate("Every 6 Days"); %>'],[168,'<% translate("Every Week"); %>']] },
					...
					<div class="section" id="backup-section">
						<form>
							<div class="input-append">
								<script type="text/javascript">
									$('#backup-section .input-append').prepend('<input size="40" type="text" maxlength="64" id="backup-name" name="backup_name" onchange="backupNameChanged()" value="router_rstats_' + nvram.et0macaddr.replace(/:/g, '').toLowerCase() + '">');
								</script>
				...
				<div class="col-sm-12">
					...
					<div class="section" id="restore-section">
						<form id="restore-form" method="post" action="bwm/restore.cgi?_http_id=<% nv(http_id); %>" encType="multipart/form-data">
							...
							<button type="button" name="f_restore_button" id="restore-button" value="<% translate("Restore"); %>" onclick="restoreButton(); return false;" class="btn"><% translate("Restore"); %> <i class="icon-upload"></i></button>
						</form><br>
					</div>
				</div>
