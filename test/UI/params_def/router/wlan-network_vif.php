<?PHP include 'header.php'; ?>
	...
	<script type="text/javascript" src="js/wireless_vif.jsx"></script>
...
if(!nvram.lan1_ifname) {
	nvram.lan1_ifname = '';
}
if(!nvram.lan2_ifname) {
	nvram.lan2_ifname = '';
}
if(!nvram.lan3_ifname) {
	nvram.lan3_ifname = '';
}
...
function refreshBandWidth(uidx)
{
	...
	nm_loaded[uidx] = 1;
		...
		wmo = {'ap': $lang.WIRELESS_ACCESS_POINT_AP,'sta': $lang.WIRELESS_CLIENT, 'wet': $lang.WIRELESS_BRIDGE};

		tabs = [['overview', $lang.OVERVIEW1]];
		...
		{ type: 'select', options: [[0,'LAN (br0)'],[1,'LAN1  (br1)'],[2,'LAN2 (br2)'],[3,'LAN3 (br3)']] }
			]);

			this.headerSet([$lang.INTERFACE, $lang.VAR_TERM_TERM_PARAMS_ACTIVATE, 'SSID', $lang.VAR_NET_MODE, $lang.BRIDGING]);
		...
		wlg.dataToView = function(data) {
			...
			if (uidx < 0) {
				ifname = (data[0].substring(0,1) == '0' ? 'ra' : 'rai') + data[0].substring(2);
			} else {
			...
			['LAN (br0)', 'LAN1 (br1)', 'LAN2 (br2)', 'LAN3 (br3)' ][data[4]]
			]);
		}
		...
		wlg.onAdd = function() {
			...
			vifs_defined.push([
				...
				(nvram['wl' + u + '_ifname']) || ((u.substring(0, 1) == '0' ? 'ra': 'rai') + u.substring(2)),	// ifname =~ wl_ifaces[uidx][0]
			...
			var e = E('footer-msg');
			e.innerHTML = $lang.WLAN_VIF_INFO1;
		...
		function earlyInit() {
			...
			for (var uidx in vifs_defined) {
				...
				for (var i = 0; i < total; ++i) {
					...
					var t = u + s;
					var v = wl_ifidxx(t);
					var w = (v < 0) ? ((u == 0 ? 'ra': 'rai') + t.substring(2)) : (wl_ifaces[v][0] + ((wl_sunit(v) < 0) ? ' (wl' + t + ')' : ''));
		...
		function init() {
			...
			if (uninit < 0) {
				E('sesdiv').innerHTML = '<i>'+ $lang.ROUTER_NOT_SUPPORTED_TIP +'.</i>';
				return;
			}
		...
		function tabSelect(name) {
			...
			// E('save-button').value = (name != 'overview') ? 'Overview' : $lang.SAVE;
			...
			for (var vidx = 0; vidx < vifs_possible.length; ++vidx) {
				...
				if (u.toString().indexOf('.') < 0) {
					...
					if (((wmode == 'wds') || (wmode == 'apwds')) && (wl_vis[vidx]._wl_channel == 1) && (E('_wl'+u+'_channel').value == '0')) {
						ferror.set('_wl'+u+'_channel', $lang.WLAN_NETWORK_INFO3, quiet || !ok);
						ok = 0;
					}
		...
		function save() {
			// if (E('save-button').value != $lang.SAVE) {
			// 	tabSelect('overview');
			// 	return;
			// }
			...
			for (vidx = 0; vidx < vifs_possible.length; ++vidx) {
				...
				if (u.indexOf('.') < 0) {
					...
					for (i = 0; i < vifs_defined.length; ++i) {
						...
						a.push(vifs_defined[i][1]);
					}
					E('_wl'+u+'_vifs').value = a.join(' ');
					E('_wl'+u+'_ifname').value = nvram['wl'+u+'_ifname'] || vifs_defined[vif][1];
				} else {
					E('_wl'+u+'_ifname').value = nvram['wl'+u+'_ifname'] || ((u.substring(0, 1) == '0' ? 'ra': 'rai') + u.substring(2));
				}
		...
		function do_pre_submit_form(fom) {
			...
			footermsg.innerHTML = $lang.SAVING + '...';
	...
	<input type="hidden" name="_nextpage" value="/#wlan-network_vif.asp">
	...
	<div class="box" id="sesdiv" style="display:none">
		<div class="heading"><script type="text/javascript">document.write($lang.MULTIPLE_SSID)</script></div>
...
/* REMOVE-END */ -->
			...
			<script type="text/javascript">
				...
				for (var i = 1; i < tabs.length; ++i) {
					...
					htmlOut += '<br>';
					htmlOut += $lang.NETWORK_INTERFACE + tabs[i][1] + $lang.IS_NOT_DEFINED + '. <br /><br />';
					htmlOut += '</div>';
					...
					var f = [];
					f.push (
						{ title: $lang.ENABLE_WIRELESS, name: 'f_wl'+u+'_radio', type: 'checkbox',
							value: (eval('nvram["wl'+u+'_radio"]') == '1') && (eval('nvram["wl'+u+'_net_mode"]') != 'disabled') },
						{ title: $lang.VAR_DEVICE_MAC, text: '<a href="#advanced-mac.asp">' + (eval('nvram["wl'+u+'_hwaddr"]') || '00:00:00:00:00:00') + '</a>' +
						...
						{ title: $lang.WIRELESS_MODE, name: 'f_wl'+u+'_mode', type: 'select',
					...
					if (u.toString().indexOf('.') < 0) {
						f.push (
							{ title: $lang.RADIO_BAND, name: 'f_wl'+u+'_nband', type: 'select', options: bands[u],
								value: eval('nvram["wl'+u+'_nband"]') || '0' == '0' ? bands[uidx][0][0] : eval('nvram["wl'+u+'_nband"]') },
							{ title: $lang.WIRELESS_NETWORK_MODE, name: 'wl'+u+'_net_mode', type: 'select',
					...
					if (u.toString().indexOf('.') < 0) {
						f.push (
							{ title: $lang.CHANNEL, name: 'wl'+u+'_channel', type: 'select', options: ghz[uidx], prefix: '<div style="display: inline-block; vertical-align: middle;" id="__wl'+u+'_channel">', 
								suffix: '</div> <button class="btn" type="button" id="_f_wl'+u+'_scan" value="'+ $lang.SCAN +'" onclick="scanButton('+u+')">Scan <i class="icon-search"></i></button> <span class="spinner" id="spin'+u+'"></span>',
								value: eval('nvram["wl'+u+'_channel"]') },
							{ title: $lang.BANDWIDTH, name: 'wl'+u+'_nbw_cap', type: 'select', options: [],
								value: eval('nvram.wl'+u+'_nbw_cap'), prefix: '<span id="__wl'+u+'_nbw_cap">', suffix: '</span>' },
							{ title: $lang.CONTROL_SIDEBAND, name: 'f_wl'+u+'_nctrlsb', type: 'select', options: [['lower',$lang.LOWER],['upper',$lang.UPPER]],
					...
					f.push (
						{ title: $lang.SECURITY_OPTIONS, name: 'wl'+u+'_security_mode', type: 'select',
							options: [['disabled',$lang.VAR_NONE],['wpa_personal','WPA Personal'],['wpa2_personal','WPA2 Personal'],['wpaX_personal','WPA / WPA2 Personal']],
							value: eval('nvram["wl'+u+'_security_mode"]') },
						{ title: $lang.ENCRYPT_TYPE, indent: 2, name: 'wl'+u+'_crypto', type: 'select',
							options: [['tkip','TKIP'],['aes','AES'],['tkip+aes','TKIP / AES']], value: eval('nvram["wl'+u+'_crypto"]') },
						{ title: $lang.SHARED_KEY, indent: 2, name: 'wl'+u+'_wpa_psk', type: 'password', maxlen: 64, size: 64, peekaboo: 1,
							suffix: ' <button class="btn" type="button" id="_f_wl'+u+'_psk_random1" value="Random" onclick="random_psk(\'_wl'+u+'_wpa_psk\')">'+ $lang.RANDOM_GENERATE +'</button>',
							value: eval('nvram["wl'+u+'_wpa_psk"]') },
						{ title: $lang.SHARED_KEY, indent: 2, name: 'wl'+u+'_radius_key', type: 'password', maxlen: 80, size: 32, peekaboo: 1,
							suffix: ' <button class="btn" type="button" id="_f_wl'+u+'_psk_random2" value="Random" onclick="random_psk(\'_wl'+u+'_radius_key\')">'+ $lang.RANDOM_GENERATE +'</button>',
							value: eval('nvram["wl'+u+'_radius_key"]') },
						{ title: $lang.GROUP_KEY_UPDATE, indent: 2, name: 'wl'+u+'_wpa_gtk_rekey', type: 'text', maxlen: 4, size: 6, suffix: ' <i>('+ $lang.VAR_SECOND +')</i>',
						...
						{ title: $lang.ENCRYPT_TYPE, indent: 2, name: 'wl'+u+'_wep_bit', type: 'select', options: [['128','128-bits'],['64','64-bits']],
							value: eval('nvram["wl'+u+'_wep_bit"]') },
						{ title: $lang.PASSWORD_SEED, indent: 2, name: 'wl'+u+'_passphrase', type: 'text', maxlen: 16, size: 20,
							suffix: ' <input type="button" id="_f_wl'+u+'_wep_gen" value="'+ $lang.GENERATE +'" onclick="generate_wep('+u+')"> <button class="btn" type="button" id="_f_wl'+u+'_wep_random" value="'+ $lang.RANDOM_GENERATE +'" onclick="random_wep('+u+')">'+ $lang.RANDOM_GENERATE +'</button>',
					...
					for (var j = 1; j <= 4; ++j) {
						f.push(
							{ title: ($lang.PPTP_CLIENT_PASSWD + j), indent: 2, name: ('wl'+u+'_key' + j), type: 'text', maxlen: 26, size: 34,
					...
					f.push(
						{ title: 'WDS', name: 'f_wl'+u+'_lazywds', type: 'select',
							options: [['0', $lang.LINK_WITH + '...'],['1',$lang.VAR_AUTO]], value: nvram['wl'+u+'_lazywds'] } );
					/* REMOVE-BEGIN */
	...
	<!-- <input type="button" value="<%translate("Save");%>" id="save-button" onclick="save()" class="btn btn-primary"></input> -->
	<!-- <input type="button" value="<%translate("Cancel");%>" id="cancel-button" onclick="javascript:reloadPage();" class="btn"></input> -->
