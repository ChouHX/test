...
class  ThinkTemplate {
    ...
    public function parse($content) {
        ...
        $content    =   preg_replace_callback('/'.$begin.'literal'.$end.'(.*?)'.$begin.'\/literal'.$end.'/is', function($match){
            return $this->parseLiteral($match[1]);
        }, $content);
        ...
        $content = preg_replace_callback('/('.$this->config['tmpl_begin'].')([^\d\s'.$this->config['tmpl_begin'].$this->config['tmpl_end'].'].+?)('.$this->config['tmpl_end'].')/is', function($match){
        ...
        foreach ($tLib->getTags() as $name=>$val){
            ...
            foreach ($tags as $tag){
                ...
                if (!$closeTag){
                    ...
                    $content = preg_replace_callback('/'.$begin.$parseTag.$n1.'\/(\s*?)'.$end.'/is', function($match)use($tagLib, $tag){
