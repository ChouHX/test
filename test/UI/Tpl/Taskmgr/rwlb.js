(function($){
    $.gf.getStaticsInfo = function(e){
        ...
        ajax(tpurl('Taskmgr','rwlbStatisticalInfo'), '', function(msg){
            msg = msg.data;
            for (var x in msg){
                $('#'+x).html(msg[x]);
            }
    ...
    $.gf.taskDetailDel = function(){
        ...
        $.confirm($lang.VAR_CONFIRM_DEL_TASK_BATCH.replace('%d',ids.length), function(){
            ajax(tpurl('Taskmgr','taskDetailDel'), {ids:ids.join(',')}, function(msg){
                $.notice(msg);
    ...
    $.gf.taskRetry = function(){
        var ids = $('#list3').jqGrid('getGridParam','selarrrow');
        if (ids.length == 0){
            $.notice(1,$lang.SELECT_TASK);
            return;
        }
        $.confirm($lang.VAR_RETRY_SINGLE_CONFIRM, function(){
            ajax(tpurl('Taskmgr','taskDetailRetry'), {ids:ids.join(',')}, function(msg){
                $.notice(msg);
                if (msg.status == 0){
                    $("#list3").trigger('reloadGrid');
                    $.gf.getStaticsInfo();
    ...
    if ($.gf.prev_page > 0){
        $.gf.cmd = localStorage.getItem('rwlb_cmd');
        $.gf.searchString = localStorage.getItem('rwlb_searchString');
        $.gf.page = localStorage.getItem('rwlb_page');
        $.gf.rowNum = localStorage.getItem('rwlb_rowNum');
    }

    var tp = null;
    $(document).ready(function(){
        tp = $('#list1').taskpaging({
            url: tpurl('Taskmgr','rwlb'),
            sidx: 'id',
            sord: 'desc',
            rowNum: 10,
            pager: '#list1_paging',
        ...
        $('.btn-task-acts button').click(function(){
            ...
            if (act == 'refresh'){
                ...
                $.confirm(tids.length > 1 ? info_arr_batch[btn_index].replace('%d',tids.length) : info_arr[btn_index], function(){
                    ajax(tpurl('Taskmgr',act), {ids:tids.join(','),is_enable:is_enable}, function(msg){
                        $.notice(msg);
                        if (msg.status == 0 && msg.data > 0) {
                            tp.reload();
                            if (act == 'taskDel') {
        ...
        document.onkeydown = function (e) {
            ...
            if ((e.ctrlKey && e.keyCode == 82) || e.keyCode == 116) {
                localStorage.removeItem('rwlb_cmd');
                localStorage.removeItem('rwlb_searchString');
                localStorage.removeItem('rwlb_page');
                localStorage.removeItem('rwlb_rowNum');
            }
        }
