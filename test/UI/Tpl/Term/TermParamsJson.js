function TermParams(type){
    ...
    this.tp_names = {
        ...
        portforward: ['id', $lang.VAR_RULE_ENABLE, $lang.SOCKET_TYPE, $lang.SRC_ADDR, $lang.EXT_PORTS, $lang.INT_PORT, $lang.INT_ADDR, $lang.VAR_RULE_DESC],
    ...
    this.tp_grids = ['new_qoslimit_rules', 'webhostfilters', 'greparam', 'greroute', 'portforward', 'portredirect', 'ipportfilterrules', 'keywordfilters', 'weburlfilters', 'modbusCmdTable', 'xtpbasic', 'l2advanced', 'ppadvanced', 'xtpschedule', 'routes_static', 'iot_topic_table'];
    this.init();
}
    ...
    $(".params-tab-content form").each(function(){
        ...
        _fields.each(function(){
            ...
            $(this).on('change', function(){
                ...
                if (this.name == 'dualsim') {
                ...
                } else if (this.name == 'ptcl'){
                    var toggle_arrs = ['mqtt_ip', 'mqtt_host', 'mqtt_port', 'mqtt_client_id', 'mqtt_username', 'mqtt_password', 'mqtt_pub_topic', 'mqtt_ver', 'mqtt_keepalive', 'mqtt_qos', 'mqtt_retain', 'mqtt_ssl_en', 'mqtt_ssl_ver', 'mqtt_ca_en'];
                    ...
                    var v = $(this).val(),
                        ...
                        elms = {
                            '-100': [],
                            'dhcp': ['mtu_enable', 'wan_aslan'],
                            'pppoe': ['ppp_username', 'ppp_passwd', 'ppp_service', 'wan_demand', 'mtu_enable', 'ppp_mlppp', 'wan_aslan'],
                            'static': ['wan_ipaddr', 'wan_netmask', 'wan_gateway', 'mtu_enable', 'wan_aslan', 'wan1_ipaddr', 'wan1_netmask', 'wan1_gateway', 'dns_1', 'dns_2'],
                ...
                if (typeof toggle_arrs != 'undefined'){
                    for (var k=0; k<toggle_arrs.length; k++){
                        $('#tpid_'+toggle_arrs[k]).parents('.form-group').toggle($(this).is(':checked') || $(this).val() == '1');
                    }
                }
            });
        });
