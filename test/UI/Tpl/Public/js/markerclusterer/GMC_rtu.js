    ...
    for (var i=0, len=me.pics.length; i<len; i++) {
        ...
        map_add_event(marker, 'click', function(e, that){
            ...
            var infoHtml ='<table class="table-condensed infoWindowTable" style="font-size:12px">\
				...
				<tr><td width="70px">'+$lang.VAR_OPERATION+':</td><td><a href="javascript:;" style="text-decoration:underline" onclick="javascript:gmc.showRtudata(\''+sn+'\')">'+$lang.RECV_DATA+'</a></td>';
            infoHtml += '</table>';
...
//查看数据
GMC.prototype.showRtudata = function(sn){
	var me = this, cols = localStorage && localStorage.getItem('lora_list3_colModel') || '';
	location.href = tpurl('Rtu','lssj','gateway_sn='+sn+'&cols='+cols);
}
