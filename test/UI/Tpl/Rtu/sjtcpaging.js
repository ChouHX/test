    ...
    obj.prototype = {
        init: function(){
            this.opt.postData = {
                ...
                searchType: 'term_transfer_config'
            }
            this.reload();
            return this;
        },
        ...
        content: function(d){
            ...
            for (var i=0,row=null,mr=''; i<d.rows.length; i++){
                ...
                str += '<div class="task" data-tid="'+row.id+'" style="'+mr+'">\
                  <div class="task_header"'+(row.is_enable != 1 ? ' style="color:red;"' : '')+'>\
                  ...
                  <div class="task_total" style="line-height: 30px;">\
                    <span class="task-count text">'+$lang.UPPER_MACHINE+': </span>\
                    <span><font>'+row.up_num+'</font></span>\
                    <span class="task-count text" style="margin-left:30px;">'+$lang.LOWER_MACHINE+': </span>\
                    <span><font>'+row.down_num+'</font></span>\
                  </div>\
                  <div class="task_total" style="line-height: 30px;">\
                    ...
                    <span class="operation">&nbsp;\
                        ...
                        <i class="fa fa-pencil" title="'+$lang.VAR_EDIT+'" onclick="$.gf.ad_edit('+row.id+',\''+row.name+'\', '+row.is_enable+', \''+row.info+'\')">&nbsp;&nbsp;</i>\
                        <i class="fa fa-remove" title="'+$lang.VAR_DEL+'" onclick="$.gf.ad_delete('+row.id+')">&nbsp;&nbsp;</i>\
                    </span>\
                  </div>\
                  <img src="'+$.gf.public_path+'images/trans_cfg.ico'+'" title="'+$lang.VAR_INFO+'" onclick="$.gf.ad_detail('+row.id+',\''+row.name+'\')" />\
                </div>';
        ...
        set_search_val: function(v){
            var me = this;
            me.opt.postData.searchType = 'term_transfer_config';
